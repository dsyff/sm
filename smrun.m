...
if ~ishandle(figurenumber)
    %05062022 -Thomas default to maximize
...
for i = 1:totpoints    
    ...
    for j = fliplr(loops(~isdummy(loops) | count(loops)==1))
        ...
        if count(j) == 1 %
            ...
            tloop(j) = now;
        elseif ~all(autochan)
            if smdata.options.skip_forced_ramp % -Thomas 2.20.2022
                pause(scandef(j).ramptime);
        ...
        if smdata.options.skip_forced_ramp %Thomas 2.20.2022
            stepWaitStart = now;
            while (now - stepWaitStart)*24*3600 < max(abs(scandef(j).ramptime))
            end
        else
...
%saveas(figurenumber,figstring,'pdf');  %thomas 07292022
