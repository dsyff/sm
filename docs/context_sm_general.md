...
- Keep real-time plot axes deterministic: 1D displays always use the loop where the channel is acquired for the x-axis, while 2D displays require that loop to be ≤ `nloops-1` and choose a distinct remaining loop for the y-axis (error when none exists).
...
- `pixel_index` must be an integer within the detector range (no rounding) and is handled through `prepareCounts(newIndex)` so a single acquisition feeds both `wavelength` and `CCD_counts` reads until the index or acquisition settings change.
- `exposure_time` channel must update the camera through `SetExposureTime` and invalidate cached spectra.
- Spectrum acquisition path: `StartAcquisition` → poll `GetStatus` until `DRV_IDLE` → `GetAcquiredData` into a `System.Int32` array → convert to `double` → mark cache fresh.
