...
function header = smguiLoopHeaderLayout(m, y)
    ...
    for k = 1:numel(labelW)
        header.label(k,:) = [x y-5 labelW(k) m.ctrlH];
        x = x + labelW(k);
...
function row = smguiSetRowLayout(m, y)
    ...
    row.channelLabel = [58 y-5 45 m.ctrlH];
    ...
    for k = 1:numel(labelW)
        row.paramLabel(k,:) = [x y-5 labelW(k) m.ctrlH];
