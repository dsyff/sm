...
if nargin > 0
   ...
   smaux.smgui.FileMenu = uimenu('Parent',smaux.smgui.figure1,...
    ...
    smaux.smgui.numloops_sth = uicontrol('Parent',smaux.smgui.nullpanel,'Style','text',...
        'String','Loops:',...
        'HorizontalAlignment','right',...
        'Position',[leftPad panYloops 40 15]);
    ...
    smaux.smgui.commenttext_sth = uicontrol('Parent',smaux.smgui.nullpanel,'Style','text',...
        'String','Comments:',...
        'HorizontalAlignment','left',...
        'Position',[leftPad panYcomm leftInnerW 20]);
    smaux.smgui.commenttext_eth = uicontrol('Parent',smaux.smgui.nullpanel,'Style','edit',...
        'String','',...
        'FontSize',8,...
        'Position',[leftPad panYcomm-130 leftInnerW 130],...
        'HorizontalAlignment','left',...
        'max',20,...
        'Callback',@Comment);
    ...
    smaux.smgui.oneDplot_sth = uicontrol('Parent',smaux.smgui.nullpanel,'Style','text',...
        'String','1D Plots',...
        'Position',[leftPad panYDisp 95 20]);
    plotListHeight = 240;
    plotGap = 10;
    plotW = floor((leftInnerW - plotGap) / 2);
    plotX2 = leftPad + plotW + plotGap;
    smaux.smgui.oneDplot_lbh = uicontrol('Parent',smaux.smgui.nullpanel,'Style','listbox',...
        'String',{},...
        'Max',10,...
        'Position',[leftPad panYDisp-plotListHeight plotW plotListHeight],...
        'Callback',@Plot);
    ...
    panYbuttons = panYDisp - plotListHeight - 40;
    buttonH = 30;
    buttonGap = buttonH / 2;
    ...
    smaux.smgui.toscans_pbh = uicontrol('Parent',smaux.smgui.nullpanel,'Style','pushbutton',...
        'String','TO SCANS',...
        'FontSize',14,...
        'Position', [leftPad panYbuttons leftInnerW buttonH],...
        'Callback',@ToScans);
        
    smaux.smgui.toqueue_pbh = uicontrol('Parent',smaux.smgui.nullpanel,'Style','pushbutton',...
        'String','TO QUEUE',...
        'FontSize',14,...
        'Position', [leftPad panYbuttons-buttonH-buttonGap leftInnerW buttonH],...
        'Callback',@ToQueue);
    
    smaux.smgui.smrun_pbh = uicontrol('Parent',smaux.smgui.nullpanel,'Style','pushbutton',...
        'String','RUN',...
        'FontSize',14,...
        'Position',[leftPad panYbuttons-2*(buttonH+buttonGap) leftInnerW buttonH],...
...
function makescanbody(varargin)
    ...
    bodyHeight = max(100, posNull(4) - m.topPad - bodyBottom);
...
function makeLoopPanel(parent, panelPos, i, setRows, recordRows, contentRows, scrollValue)
    ...
    smaux.smgui.loopvars_addchan_pbh(i) = uicontrol("Parent", smaux.smgui.loop_panels_ph(i), ...
        "Style", "pushbutton", ...
        "String", "Add Channel", ...
        "Position", [25 headerY 100 m.ctrlH], ...
    ...
    makeLoopHeaderEdit(i, 1, "Points:", [165 headerY-5 50 m.ctrlH], [215 headerY 50 m.ctrlH], numpoints, "");
    makeLoopHeaderEdit(i, 4, "Start Wait:", [335 headerY-5 55 m.ctrlH], [395 headerY 35 m.ctrlH], loopValueOrZero(smscan.loops(i), "startwait"), "After setting first value of loop, waits this amount of time (s)");
    makeLoopHeaderEdit(i, 2, "Step time:", [435 headerY-5 55 m.ctrlH], [495 headerY 55 m.ctrlH], loopValueOrZero(smscan.loops(i), "ramptime"), "(s) Time when ramping between each point");
    makeLoopHeaderEdit(i, 3, "Wait (s):", [555 headerY-5 55 m.ctrlH], [615 headerY 55 m.ctrlH], loopValueOrZero(smscan.loops(i), "waittime"), "(s) Wait before getting data");
    ...
    smaux.smgui.looprecord_sth(i) = uicontrol("Parent", smaux.smgui.loop_content_ph(i), ...
        ...
        "String", "Record:", ...
        "HorizontalAlignment", "center", ...
        "Position", [5 recordStartY-5 50 m.ctrlH]);
...
function m = smguiLayoutMetrics()
    ...
    m.maxContentRows = 5;
    m.constCols = 4;
    m.constColSpacing = 158;
    m.recordCols = 7;
    m.recordColSpacing = 82;
    m.setColSpacing = 90;
end
...
function makeloopchannelset(i,j) 
    ...
    pos = [5 size(4)-m.rowH*j+m.ctrlPadY 0 0];
    w = [90 40 40 40 40 40]; % widths
    h = m.ctrlH * ones(1, 11); % heights
    s = m.setColSpacing; %spacing
...
function makeloopgetchans(i, recordStartY)
    ...
    recordCols = m.recordCols;
    recordRowSpacing = m.rowH;
    for j=1:numgetchans
        try
            % Find position in dropdown list, not smdata.channels indices
        ...
        recordPos = [60 + m.recordColSpacing * mod(j-1, recordCols), ...
            recordStartY - recordRowSpacing * floor((j-1) / recordCols), 80, 20];
    ...
    recordPos = [60 + m.recordColSpacing * mod(j, recordCols), ...
        recordStartY - recordRowSpacing * floor(j / recordCols), 80, 20];
