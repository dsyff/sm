function [data, stopped] = runSafeScanCore_(rack, scanObj, scanControlToEngine, engineToClient, requestId, logFcn)
    ...
    function TF = isStopControl(ctl)
        TF = isCurrentControl(ctl) && ctl.type == "stop";
        if TF && isfield(ctl, "message") && strlength(string(ctl.message)) > 0
